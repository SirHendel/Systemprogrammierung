...
int main(int argc,char* argv[]){
    ...
    struct stat stats;  // stat struct Deklarieren 
    DIR *d;             // der Datentyp Dir Representiert ein Direnctory stream 
    struct dirent *dir; // Der pointer des Dir Datentyps wird mit dem dirent erweitert für mehr informationen
    ...
    //checkt die lange und inhalt der übergebenen Variable 
    

    if(isdir(argv[1])){ // checkt ob es eine Direnctory ist

        d = opendir(argv[1]);  // setzt dem stream auf unserm 

        chdir(argv[1]); // bewegt das arbeit direnctory zu unserem
